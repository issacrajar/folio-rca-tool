...
// Transaction Flattener — flatten folioTransactionDetails across all folios
 ...
 */
export function flattenTransactions(folios: FolioInput[]): FlattenResult {
  const transactions: Transaction[] = [];
  const byLineItemNo = new Map<string, Transaction>();
  const perWindowSummary: WindowSummary[] = [];
