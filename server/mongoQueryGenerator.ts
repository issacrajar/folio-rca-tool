...
const tenantLookup = new Map<string, TenantInfo>();

/**
 * Load tenantList.xlsx at startup and build the PropertyCode lookup map.
 */
export function loadTenantList(): void {
  ...
  try {
    ...
    for (let i = headerIdx + 1; i < rawRows.length; i++) {
      const row = rawRows[i];
      if (!row || !row[colIdx.propertyCode]) continue;
 ...
 */
export function parseFolioId(folioId: string): { propertyCode: string; chargePostingSequenceNumber: string } {
 ...
 */
export function lookupTenant(propertyCode: string): TenantInfo | undefined {
 ...
 */
export function generateMongoQuery(
  tenantId: string,
  propertyId: string,
  accountId: string
): { queryString: string; queryObject: any[] } {
  const pipeline = [
    {
      $match: {
        tenantId,
 ...
 */
export function generateAccountLookupQuery(tenantId: string, chargePostingSequenceNumber: string): string {
 ...
 */
export function autoGenerateQueries(folioTransactions: any[]): {
  propertyCode: string;
  ...
  if (!propertyCode) throw new Error(`Could not extract PropertyCode from folioId: ${folioId}`);
  if (!chargePostingSequenceNumber) throw new Error(`Could not extract chargePostingSequenceNumber from folioId: ${folioId}`);
