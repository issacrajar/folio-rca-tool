...
// TransformOut Simulator — replicates FolioResendHandler.transformOut() logic
 ...
 */
export function simulateTransformOut(graphResponse: any, accountType: string): TransformOutResult {
  const trace: TraceEntry[] = [];
  const emptyFolios: string[] = [];
  ...
  let folioTransactions: any[] = [];
  const addedTransactionIds = new Set<string>();
...
function removeParentFromChildren(transactions: any[], parentId: string): void {
 ...
 */
export function simulateFolioOutTransform(transaction: any): any {
  ...
  const details = type === "GROUP" && transaction.childTransactions
    ? transaction.childTransactions.flatMap((c: any) => c.transactionDetails || [])
