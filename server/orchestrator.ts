...
// ═══════════════════════════════════════════════════════════════
// ORCHESTRATOR ENGINE — Human-in-the-loop execution controller
// ═══════════════════════════════════════════════════════════════

import { getRules } from "./userRules.js";
...
export interface PlannerDecision {
  ...
  description: string;
  intent: string;
  queryType?: string; // e.g., "ledgerTransactionById", "groupTransaction", etc.
...
export interface Rule {
  id: string;
  condition: string; // e.g., "step.type === 'graph_query' && step.intent.includes('payment')"
  action: "force_approve" | "force_reject" | "modify_query" | "skip" | "require_approval" | "inject_condition";
  modification?: string; // for modify_query
  priority: number;
}

let sessionRules: Rule[] = [];

export function loadRuleSheet(rules: Rule[]) {
  sessionRules = rules.sort((a, b) => b.priority - a.priority);
}

export function getSessionRules(): Rule[] {
  return sessionRules;
}

function applyRulesToStep(step: OrchestratorStep): { overrides: string[]; action?: "approve" | "reject" | "skip"; modifiedQuery?: string } {
  const overrides: string[] = [];
  let finalAction: "approve" | "reject" | "skip" | undefined;
    ...
    try {
      ...
      switch (rule.action) {
        ...
        case "skip":
          ...
          break;
        case "modify_query":
          if (rule.modification) modifiedQuery = rule.modification;
          break;
...
function evaluateRuleCondition(condition: string, step: OrchestratorStep): boolean {
  try {
...
// ─── Planner ──────────────────────────────────────────────────
function planNextStep(session: OrchestratorSession): PlannerDecision | null {
...
function detectAccountType(folioTransactions: any[]): string {
  if (!folioTransactions?.length) return "guest";
  const type = folioTransactions[0]?.folioType?.folioTypeCode;
  if (type === "GS") return "guest";
  if (type === "GP") return "group";
  if (type === "HA") return "house";
  return "guest";
}
 ...
 */
export function createSession(request: string, context: Record<string, any>): OrchestratorSession {
 ...
 */
export async function advanceSession(sessionId: string): Promise<{ session: OrchestratorSession; pendingStep?: OrchestratorStep }> {
  ...
  while (true) {
    ...
    // For graph/mongo queries — generate the query and pause for approval
     if (step.type === "graph_query") {
       ...
       step.variables = buildVariablesTemplate(accountType, params as any);
...
export async function respondToStep(
  ...
  modification?: string,
  responseData?: any
): Promise<{ session: OrchestratorSession; pendingStep?: OrchestratorStep }> {
  const session = sessions.get(sessionId);
  if (!session) throw new Error(`Session ${sessionId} not found`);
 ...
 * Get a session by ID.
 */
export function getSession(sessionId: string): OrchestratorSession | undefined {
  return sessions.get(sessionId);
}

// ─── Internal Helpers ─────────────────────────────────────────

async function executeMongoStep(session: OrchestratorSession, step: OrchestratorStep) {
  try {
...
async function executeInternalStep(session: OrchestratorSession, step: OrchestratorStep, decision: PlannerDecision) {
...
function extractQueryParams(context: Record<string, any>): Record<string, string> {
  const ft = context.folioTransactions;
  if (!ft?.length) return {};
  const fid = ft[0].folioId || "";
  const confIds = ft[0].confirmationIds || [];
  const acrsConf = confIds.find((c: any) => c.provider === "ACRS");
  return {
