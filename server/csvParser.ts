 ...
 */
function deriveLineItemNo(transactionId: string): string {
 ...
 */
export function parseCsvBuffer(buffer: Buffer, filename?: string): { rows: CsvRow[]; errors: string[] } {
  const errors: string[] = [];
  ...
  try {
    ...
    const rows: CsvRow[] = jsonData.map((item) => {
      let lineItemNo: string;
