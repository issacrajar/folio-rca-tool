...
function resolveTenant(extracted) {
...
const TransTypes = { NEW: "NEW", SET: "SET", ADJ: "ADJ", PKG: "PKG" };
...
function getTransactionType(folioType, type, originalFolioLineType) {
    if (folioType === FolioTypeCode.TRANSFER &&
...
function getFolioAmount(state, transaction) {
    let baseAmount;
    ...
    if (state.orginalFolioType === FolioTypeCode.PAYMENT ||
    ...
    if (state.folioType === FolioTypeCode.TRANSFER &&
        state.transType === TransTypes.SET &&
        transaction.transferReferenceId &&
        !state.arNumber) {
        baseAmount = -Math.abs(baseAmount);
    }
    if (state.originalFolioLineType === FolioTypeCode.PAYMENT &&
...
app.post("/api/compare", upload.single("csv"), (req, res) => {
    try {
        ...
        if (typeof folioTransactions === "string") {
            try {
                folioTransactions = JSON.parse(folioTransactions);
            }
            catch {
                return res.status(400).json({ error: "Invalid folioTransactions JSON" });
            }
        }
...
app.post("/api/resend-query", (req, res) => {
    try {
        ...
        if (!accountType)
            return res.status(400).json({ error: "accountType required" });
...
app.post("/api/construct-payload", (req, res) => {
    try {
        ...
        const payload = survivingTransactions.map((tx) => simulateFolioOutTransform(tx));
...
app.post("/api/verify-account", async (req, res) => {
    ...
    const reqTimeout = setTimeout(() => {
        if (!res.headersSent)
            res.status(504).json({ error: "verify-account timed out after 55s — MongoDB may be unreachable or OIDC auth is pending" });
    }, 55_000);
    try {
        const { folioTransactions } = req.body;
        if (!folioTransactions)
            return res.status(400).json({ error: "folioTransactions is missing" });
        if (!Array.isArray(folioTransactions))
            return res.status(400).json({ error: "folioTransactions must be an array" });
        if (folioTransactions.length === 0)
            return res.status(400).json({ error: "folioTransactions array is empty" });
...
app.post("/api/execute-mongo-query", async (req, res) => {
    ...
    const reqTimeout = setTimeout(() => {
        if (!res.headersSent)
            res.status(504).json({ error: "execute-mongo-query timed out after 55s — query may be too broad or MongoDB is unreachable" });
    }, 55_000);
    try {
...
app.post("/api/execute-transfer-query", async (req, res) => {
    const reqTimeout = setTimeout(() => {
        if (!res.headersSent)
            res.status(504).json({ error: "execute-transfer-query timed out after 55s" });
    }, 55_000);
    try {
...
app.post("/api/resolve-missing", (req, res) => {
    try {
        ...
        const extractFromFolios = (folios) => {
        ...
        if (allGraphTransactions.length === 0) {
            const deepSearch = (obj, depth) => {
                if (!obj || depth > 10 || typeof obj !== "object")
                    return;
        ...
        const debugKeys = (obj, prefix, depth) => {
            if (!obj || typeof obj !== "object" || depth > 3)
                return [];
            const keys = [];
            for (const [k, v] of Object.entries(obj)) {
                const path = prefix ? `${prefix}.${k}` : k;
                keys.push(`${path} (${Array.isArray(v) ? "array[" + v.length + "]" : typeof v})`);
                if (typeof v === "object" && v !== null && !Array.isArray(v)) {
                    keys.push(...debugKeys(v, path, depth + 1));
                }
                else if (Array.isArray(v) && v.length > 0) {
                    keys.push(...debugKeys(v[0], `${path}[0]`, depth + 1));
                }
            }
            return keys;
        };
        ...
        const allLineItemNos = [];
        const graphTxByLineItemNo = new Map();
        const indexTxDetails = (tx) => {
        ...
        // Missing set
        const missingSet = new Set((missingLineItemNos || []).map((n) => String(n).padStart(10, "0")));
        // CSV lookup
        const csvByLineItemNo = new Map();
        if (csvRows) {
        ...
        for (const missingLin of missingSet) {
            const graphTx = graphTxByLineItemNo.get(missingLin);
            if (!graphTx)
                continue;
            const csvRow = csvByLineItemNo.get(missingLin);
            ...
            if (csvRow) {
                ...
                rawAmount = details.reduce((s, d) => s + Number(d.amount || 0), 0);
            }
            ...
            const isRouted = graphTx.isRouted ?? details.some((d) => d.isRouted) ?? false;
            ...
            if (graphTx.postedPropertyDate)
                entry.transactionTS = graphTx.postedPropertyDate;
            // businessTS from graph: postedDateTime (FolioOutHandler line ~982)
            if (graphTx.postedDateTime)
                entry.businessTS = graphTx.postedDateTime;
            ...
            if (roomNum)
                ...
                delete entry.roomNumber; // don't inherit template's room
            ...
            if (folioMeta.resCloseDate)
                entry.resCloseDate = folioMeta.resCloseDate;
            if (folioMeta.groupCode != null)
                entry.groupCode = folioMeta.groupCode;
            if (folioMeta.groupCreateTS != null)
                entry.groupCreateTS = folioMeta.groupCreateTS;
            ...
            entry.posFlag = isPOS;
            if (isPOS)
                entry.posChkRefNo = detail.checkNumber || graphTx.checkNumber;
            ...
            if (refNum)
                entry.transRefNo = refNum;
            else
                delete entry.transRefNo;
            ...
            const needsTransferDetails = !isTaxExempted && ((ledgerType === "TRANSFER" && transType !== "SET") ||
...
app.post("/api/deep-reference-analysis", async (req, res) => {
    const reqTimeout = setTimeout(() => {
        if (!res.headersSent)
            res.status(504).json({ error: "deep-reference-analysis timed out after 5 minutes — too many sequential DB lookups; try a smaller dataset" });
    }, 300_000);
    try {
        const { folioTransactions, csvData } = req.body;
        if (!folioTransactions || !csvData) {
        ...
        const extractFirstTenDigits = (id) => {
            if (!id)
                return null;
        ...
        // Build lookup maps
        const csvByLineItemNo = new Map();
        const csvByTransactionId = new Map();
        for (const row of csvData) {
            const lin = String(row.lineItemNo).padStart(10, "0");
            csvByLineItemNo.set(lin, row);
            if (row.transactionId)
                csvByTransactionId.set(String(row.transactionId), row);
        }
        ...
        const dbFetchedRows = new Map();
        /** Fetch rows from DB for the given transactionIds (skips already-known IDs). */
        const fetchFromDb = async (ids) => {
            if (!tenantId || !propertyId || !ids.length)
                return;
            ...
            try {
                ...
                for (const row of rows) {
                    if (row.transactionId)
                        dbFetchedRows.set(String(row.transactionId), row);
                }
            }
            catch (e) {
        ...
        const getRow = (transactionId) => csvByTransactionId.get(transactionId) || dbFetchedRows.get(transactionId);
        /** Return all rows from csvData + DB cache, deduplicated by transactionId. */
        const getAllRows = () => {
            const seen = new Set();
            const rows = [];
            for (const row of [...csvData, ...Array.from(dbFetchedRows.values())]) {
        ...
        const taxReferenceResults = [];
        const referenceIdResults = [];
        const REF_TYPES = [
        ...
        if (tenantId && propertyId) {
            const missingLineItemNos = [];
            for (const folio of folioTransactions) {
                if (!Array.isArray(folio.folioTransactionDetails))
                    continue;
                for (const folioTran of folio.folioTransactionDetails) {
                    if (!Array.isArray(folioTran.folioTransferDetails) || folioTran.folioTransferDetails.length === 0)
                        continue;
        ...
        for (const folio of folioTransactions) {
            ...
            for (const folioTran of folio.folioTransactionDetails) {
                ...
                for (const transferDetail of folioTran.folioTransferDetails) {
                    ...
                    let correctTrnsfrFromLineItemNo = null;
                    let resolution = "";
                    // ══════════════════════════════════════════════════════════════════════
                    ...
                    if (!correctTrnsfrFromLineItemNo && !csvRow.taxReferenceId && tenantId && propertyId && csvRow.transactionId) {
                        ...
                        try {
                            ...
                            if (a2DocLines.length > 0) {
                                ...
                                const a2ItemMatch = a2DocLines.find(r => r.itemId === csvRow.itemId &&
                    ...
                    if (!correctTrnsfrFromLineItemNo && csvRow.taxReferenceId) {
                        ...
                        if (parentRow) {
                            ...
                            if (parentRefId && parentRefType) {
                                ...
                                let groupedRows = getAllRows().filter(r => r.taxReferenceId != null && String(r.taxReferenceId).trim() === parentRefId);
                                ...
                                const matchingRow = groupedRows.find(r => r.itemId === csvRow.itemId && String(r.transactionId) !== String(csvRow.transactionId));
                                if (matchingRow) {
                                ...
                                if (tenantId && propertyId && parentRow?.transactionId) {
                                    try {
                                        ...
                                        const bDocMatchedLine = bDocLines.find(r => r.itemId === csvRow.itemId &&
                                            String(r.transactionId) !== String(csvRow.transactionId) &&
                                            (r.sourceFolioLineItemId || r.transferReferenceId));
                                        if (bDocMatchedLine) {
                                            ...
                                            if (bDocComputed) {
                                                ...
                                                resolution = `taxReferenceId="${taxRefId}" → parent document folioLine itemId="${csvRow.itemId}" match → ${bDocRefField}="${bDocRefId}" → first10digits="${bDocComputed}"`;
                                                ...
                                                taxReferenceResults.push({
                                                    ...
                                                    correctTrnsfrFromLineItemNo: bDocComputed,
                                                    source: "parentDocumentFolioLineByItemId",
                                                });
                                            }
                                        }
                                        else {
                                ...
                                if (!bDocMatchFound) {
                                    ...
                                    const fallbackRows = getAllRows().filter(r => (r.taxReferenceId != null && String(r.taxReferenceId).trim() === taxRefId) ||
                                        String(r.transactionId) === taxRefId);
                                    detailDebug.push(`    Fallback group size: ${fallbackRows.length}`);
                                    const matchingRow = fallbackRows.find(r => r.itemId === csvRow.itemId && String(r.transactionId) !== String(csvRow.transactionId));
                                    if (matchingRow) {
                                        const computed = extractFirstTenDigits(String(matchingRow.transactionId));
                                        correctTrnsfrFromLineItemNo = computed;
                                        ...
                                        if (!correctTrnsfrFromLineItemNo && tenantId && propertyId && parentRow?.transactionId) {
                                            ...
                                            try {
                                                ...
                                                const parentHistRow = parentHistRows.find((r) => String(r.transactionId) === String(parentRow.transactionId));
                                                ...
                                                if (parentSourceId) {
                                                    ...
                                                    let b3GroupRows = getAllRows().filter(r => r.taxReferenceId != null && String(r.taxReferenceId).trim() === parentSourceId);
                                                    ...
                                                    const b3MatchingRow = b3GroupRows.find(r => r.itemId === csvRow.itemId && String(r.transactionId) !== String(csvRow.transactionId));
                                                    if (b3MatchingRow) {
                                                        ...
                                                        if (!b3RefId && tenantId && propertyId) {
                                                            ...
                                                            try {
                                                                ...
                                                                const b3HistRow = b3HistRows.find((r) => String(r.transactionId) === String(b3MatchingRow.transactionId));
                                                                if (b3HistRow) {
                                                        ...
                                                        try {
                                                            ...
                                                            const b3bMatchingRow = docLines.find(r => r.itemId === csvRow.itemId && String(r.transactionId) !== String(csvRow.transactionId));
                                                            if (b3bMatchingRow) {
                                                                ...
                                                                if (!b3bRefId && tenantId && propertyId) {
                                                                    ...
                                                                    try {
                                                                        ...
                                                                        const b3bHistRow = b3bHistRows.find((r) => String(r.transactionId) === String(b3bMatchingRow.transactionId));
                                                                        if (b3bHistRow) {
...
app.post("/api/orchestrator/start", async (req, res) => {
    try {
        ...
        const context = { folioTransactions };
        if (csvData)
