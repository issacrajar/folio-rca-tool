...
import { applyBuiltInRules } from "./builtInRules.js";
 ...
 */
export function correctPayload(transactions, csvRows, originalFolios) {
    ...
    for (const txn of transactions) {
        ...
        if (csvRow) {
            const ruleResult = applyBuiltInRules(csvRow, current);
        ...
        traces.push(trace);
        correctedByLineItemNo.set(txn.lineItemNo, current);
    }
    // Rebuild corrected payload in original folio structure
    let correctedPayload;
    if (originalFolios) {
        correctedPayload = originalFolios.map((folio) => {
            const correctedDetails = (folio.folioTransactionDetails ?? []).map((txn) => {
    ...
    for (const folio of correctedPayload) {
        ...
        const sumByTransLinkId = new Map();
        for (const txn of details) {
            if (txn.transLinkId && txn.transType !== "PKG" && txn.transType !== "SET") {
                sumByTransLinkId.set(txn.transLinkId, (sumByTransLinkId.get(txn.transLinkId) ?? 0) + (txn.transactionAmt?.value ?? 0));
            }
        }
