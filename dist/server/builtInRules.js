 ...
 */
function traceCodePath(csvRow, transaction, expected, actual) {
    const traces = [];
...
export function applyBuiltInRules(csvRow, transaction) {
    let matched;
    ...
    const isCorrect = transaction.transType === expectedTransType &&
 ...
 */
export function validatePkgTransaction(pkgTransaction, allTransactions) {
    const linkedTransactions = allTransactions.filter((t) => t.transLinkId === pkgTransaction.lineItemNo);
